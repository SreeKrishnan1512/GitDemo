...
#For finding the row and column of fruit name

for i in range(1,active.max_column+1):
    if active.cell(row=1,column=i).value=="fruit_name":
        row_column_Price=i
        print(f"Price lies in row 1 and {row_column_Fruit} column")
    
print(row_column_Fruit)
...
for i in range(1,active.max_row+1):
    ...
    for j in range(1,active.max_column+1):
        ...
        Dict[active.cell(row=i,column=row_column_Fruit).value]= active.cell(row=i,column=row_column_Price).value
